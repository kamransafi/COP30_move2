...
#'
#' # For larger extents, tiling is triggered automatically:
#' # res <- get_COP30_move2(large_move2_object, max_pixels_per_request = 5e6)
#' }
#'
#' @export
#' Download and annotate Copernicus DSM (30 m) for a move2 object
...
.get_dsm_tiled <- function(x,
  ...
  x_wgs <- sf::st_transform(x, 4326)
  tiles <- .make_tiles(global_bbox, tile_dx = tile_dx, tile_dy = tile_dy)
  ...
  for (i in seq_len(nrow(tiles))) {
    ...
    # Request DSM for this tile
    res_tile <- .get_dsm_single_request(
      x             = x_sub,
      bbox_override = tile_bbox,
      method        = method,
      plot_overview = FALSE,
      dem_file      = NULL,
      key_service   = key_service
